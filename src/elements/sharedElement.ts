...
export type ResizeableElements =
  | DraggableButton
  | DraggablePanel
  | DraggableCanvas
  | DraggableCollectionPanel
...
export class ElementSharedFuncs {
  ...
  public static startResize(
    ...
    if (stopPropagation) e.stopPropagation(); // Prevent parent handlers
  ...
  public static resize(e: MouseEvent, classElement: ResizeableElements): void {
    ...
    const containerRect: DOMRect =
      classElement.container.getBoundingClientRect();
    ...
    if (selectedElement) {
      if (selectedElement !== element) {
        const selectedClassElement =
          GeneralUtil.elementToClassElement(selectedElement)!;
  ...
  public static startDrag(
    e: MouseEvent,
    classElement: GlobalElementMapValue,
  ): void {
    ...
    const parentClassElement =
      GeneralUtil.elementToClassElement(parentElement)!;
    if (
      isGridableElement(parentClassElement) &&
  ...
  public static drag(
    ...
    if (config.settings.grid_lock.value) {
      ...
      const nearestGridX =
        Math.round(centerPoint[0] / gridCellWidth) * gridCellWidth;
      const nearestGridY =
        Math.round(centerPoint[1] / gridCellHeight) * gridCellHeight;
      const closestPoint: [number, number] = [nearestGridX, nearestGridY];

      const distanceToPoint = MathUtil.getDistanceVector2(
        centerPoint,
        closestPoint,
      );
    ...
    if (config.settings.boundary_constraints!.value) {
      ...
      newLeft = Math.max(
        0,
        Math.min(newLeft, containerRect.width - mainElement.offsetWidth),
      );
      newTop = Math.max(
        0,
        Math.min(newTop, containerRect.height - mainElement.offsetHeight),
      );
    }
   ...
   */
  public static updateCenterCirclePosition(
    classElement: GridableElements,
  ): void {
    const rect: DOMRect = classElement
      .getMainHTMLElement()
  ...
  public static grid(showGrid: boolean, classElement: GridableElements): void {
    if (!showGrid) {
      ...
      classElement.gridElement!.style.setProperty(
        "--grid-cols",
        String(config.settings.grid_lock_columns.value),
      );
      classElement.gridElement!.style.setProperty(
        "--grid-rows",
        String(config.settings.grid_lock_rows.value),
      );
    }
  }
