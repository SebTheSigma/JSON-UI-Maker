...
class DatabaseManager {
  ...
  private readonly STORAGE_KEY = 'json_ui_database';
  private readonly MANUAL_DATA_KEY = 'json_ui_manual_data';
  private presetsCache: Preset[] = [];
  ...
  private restoreDataToDatabase(): void {
    try {
      // Restore users
  ...
  private saveManualData(): void {
    try {
      ...
      this.presetsCache = (presetsQuery.values || []).map((row: any) => ({
        id: row[0],
        user_id: row[1],
        name: row[2],
        png_path: row[3],
      ...
      this.usersCache = (usersQuery.values || []).map((row: any) => ({
        id: row[0],
        username: row[1],
        password_hash: row[2],
        created_at: row[3]
  ...
  // Preset management
  async createPreset(userId: number, name: string, pngPath: string, jsonPath: string, isPublic: boolean = false): Promise<Preset> {
  ...
  async getUserPresets(userId: number): Promise<Preset[]> {
    try {
  ...
  async updatePreset(presetId: number, updates: { is_public?: boolean; name?: string }): Promise<void> {
    try {
...
export function loadSqlJs(): Promise<void> {
