...
export class PresetManagementModal {
  ...
  private createPresetItemHtml(preset: any): string {
  ...
  public startEditPreset(presetId: number): void {
    ...
    const input = document.getElementById(`editPresetName_${presetId}`) as HTMLInputElement;
    input?.focus();
    input?.select();
  }

  public async savePresetName(presetId: number): Promise<void> {
    const input = document.getElementById(`editPresetName_${presetId}`) as HTMLInputElement;
    if (!input) return;
  ...
  public cancelEditPreset(presetId: number): void {
    // Reload the presets to restore original state
    this.loadPresets();
  }

  public async togglePresetVisibility(presetId: number): Promise<void> {
    try {
