...
export async function loadTexturePresetsModal() {
    ...
    new Promise((): void => {
        submit.onclick = async () => {
            ...
            // Handle user presets
            const presetCheckboxes = form.querySelectorAll('input[type="checkbox"][data-preset-id]') as NodeListOf<HTMLInputElement>;
            ...
            for (let checkbox of Array.from(presetCheckboxes)) {
                if (checkbox.checked) {
                    ...
                    if (presetType === 'user' || presetType === 'public') {
                        ...
                        const userPresets = await (window as any).presetManager.getUserPresets();
                        const preset = userPresets.find((p: any) => p.id === presetId);
                        if (!preset) {
                        ...
                        try {
                            ...
                            const files: { name: string; type: 'png' | 'json'; key: string }[] = JSON.parse(metadataStr);
...
export async function addUserPresetsSection(form: HTMLElement, userId: number) {
