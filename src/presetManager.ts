...
class PresetManager {
  private readonly STORAGE_PREFIX = 'json_ui_preset_';

  async uploadPreset(files: FileList, presetName: string, isPublic: boolean = false): Promise<{ success: boolean; message: string }> {
    ...
    console.log('Existing presets before upload (names):', existingPresets.map((p: Preset) => p.name));
    ...
    if ((window as any).dbManager) {
      const directDbPresets = await (window as any).dbManager.getUserPresets(user.id);
      console.log('🔍 Direct database query before upload:', directDbPresets.length, directDbPresets.map((p: Preset) => p.name));
    }
    ...
    try {
      const storedFiles: { name: string; type: 'png' | 'json'; key: string }[] = [];
      ...
      const preset = await (window as any).dbManager.createPreset(
        user.id,
        presetName,
        metadataKey, // Store metadata key as png_path
  ...
  async getUserPresets(): Promise<Preset[]> {
    ...
    try {
      ...
      if ((window as any).dbManager && typeof (window as any).dbManager.save === 'function') {
        await (window as any).dbManager.save();
      }
      
      const presets = await (window as any).dbManager.getUserPresets(user.id);
      console.log('✅ Retrieved', presets.length, 'presets from database:', presets.map((p: Preset) => p.name));
      return presets;
    } catch (error) {
  ...
  async getPublicPresets(): Promise<Preset[]> {
    try {
      return await (window as any).dbManager.getPublicPresets();
    } catch (error) {
  ...
  async makePresetPublic(presetId: number): Promise<{ success: boolean; message: string }> {
    const user = authManager.getCurrentUser();
    if (!user) {
    ...
    try {
      ...
      if ((window as any).dbManager && typeof (window as any).dbManager.updatePreset === 'function') {
        await (window as any).dbManager.updatePreset(presetId, { is_public: !preset.is_public });
      } else {
        // Fallback: delete old and create new with updated visibility
        if ((window as any).dbManager && typeof (window as any).dbManager.deletePreset === 'function') {
          await (window as any).dbManager.deletePreset(presetId);
        }
        
        await (window as any).dbManager.createPreset(
          user.id,
          preset.name,
          preset.png_path,
          preset.json_path,
  ...
  getPresetData(preset: Preset): { pngData: string; jsonData: any } | null {
    try {
  ...
  private isValidNinesliceJson(data: any): boolean {
   ...
   */
  private async loadPresetTexturesIntoSystem(storedFiles: { name: string; type: 'png' | 'json'; key: string }[]): Promise<void> {
    try {
      const loadedImages: { [key: string]: { png?: ImageData; json?: any } } = {};
      ...
      for (const [imageName, data] of Object.entries(loadedImages)) {
        if ((window as any).images) {
          (window as any).images.set(imageName, data);
   ...
   */
  private async createImageDataFromBase64(base64Data: string): Promise<ImageData> {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.onload = () => {
