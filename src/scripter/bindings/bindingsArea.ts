...
export class BindingsArea {
  ...
  },
  {
    ...
    public static doubledLetters = new Map([
        ["[", "]"],
        ["{", "}"],
        ['"', '"'],
        ["'", "'"],
    ...
    static format(): void {
        if (this.errorMessage.style.visibility === "visible") return;
    ...
    public static updateWarningLabel(): boolean {
        try {
    ...
    public static handleKeyboardInput(e: KeyboardEvent): void {
        const start = this.bindingsTextArea.selectionStart;
        const end = this.bindingsTextArea.selectionEnd;
        ...
        if (e?.key == '"' && ta.value[start - 2] != ":") {
            ...
            if (this.BindingsTextPrompt.attached) {
                if (this.BindingsTextPrompt.data.type == "binding_keys") this.checkForDeletedChars('"');
                else if (this.BindingsTextPrompt.data.type == "source_property_name") this.checkForDeletedChars("#");
            }
        ...
        setTimeout(() => {
            if (!this.BindingsTextPrompt.attached) return;
            if (this.BindingsTextPrompt.data.type == "binding_keys") this.filterBindingKeys();
            if (this.BindingsTextPrompt.data.type == "source_property_name") this.filterSourcePropertyNames();
        }, 0);
    }

    public static checkForDeletedChars(char: string): void {
        const ta = this.bindingsTextArea;
    ...
    public static filterSourcePropertyNames(): void {
        ...
        this.BindingsTextPrompt.addTextOptions(
            searchedProps.map((n) => `#${n}`),
            this.insertSourcePropertyName.bind(this)
        );
    }

    public static filterBindingKeys(): void {
        const currentSpeechMarkValues = this.bindingsTextArea.value.substring(0, this.bindingsTextArea.selectionStart).split('"');
        if (currentSpeechMarkValues.length === 0) return;
     ...
     * @returns {void}
     */
    public static insertSourcePropertyName(textToInsert: string): void {
        const ta = this.bindingsTextArea;
     ...
     * @returns {void}
     */
    public static insertBindingKey(textToInsert: string): void {
        const ta = this.bindingsTextArea;
        const start = ta.selectionStart;
